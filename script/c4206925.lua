...
function s.zarctg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g1=rg:IsAttribute(ATTRIBUTE_DARK,fc,sumtype,tp) and c:IsType(TYPE_FUSION,fc,sumtype,tp)
	local g2=rg:IsAttribute(ATTRIBUTE_DARK,fc,sumtype,tp) and c:IsType(TYPE_SYNCHRO,fc,sumtype,tp)
	local g3=rg:IsAttribute(ATTRIBUTE_DARK,fc,sumtype,tp) and c:IsType(TYPE_XYZ,fc,sumtype,tp)
	local g4=rg:IsAttribute(ATTRIBUTE_DARK,fc,sumtype,tp) and c:IsType(TYPE_PENDULUM,fc,sumtype,tp)
	local g=g1:Clone()
	g:Merge(g2)
	g:Merge(g3)
	g:Merge(g4)
