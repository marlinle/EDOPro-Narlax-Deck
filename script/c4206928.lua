-- The Gates to the Nightmare World
...
function s.initial_effect(c)
    ...
    e2:SetTarget(aux.TargetBoolFunction(Card.IsRace,RACE_ZOMBIE+RACE_FIEND))
    e2:SetValue(500)
    c:RegisterEffect(e2)
    --def up
    local e3=e2:Clone()
...
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then
        local g=Duel.GetMatchingGroup(Card.IsSetCard,tp,LOCATION_DECK,0,nil,0x420)
