...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Special Summon Supreme Nightmare King Rexne
...
function s.zarcspfilter(c,e,tp,sg)
	return c:IsCode(4206920) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.GetLocationCountFromEx(tp,tp,sg,c)>0
end
function s.chk(c,sg,g,type,...)
	if not (c:IsType(type) or c:IsSetCard(0x420c) ) then return false end
	local res
	if ... then
		g:AddCard(c)
...
function s.rescon(sg,e,tp,mg)
	return sg:IsExists(s.chk,1,nil,sg,Group.CreateGroup(),TYPE_FUSION,TYPE_PENDULUM,TYPE_XYZ,TYPE_SYNCHRO) and Duel.GetLocationCountFromEx(tp,tp,sg,TYPE_FUSION)>0
end
function s.zarctg(e,tp,eg,ep,ev,re,r,rp,chk)
  ...
  local g1=rg:Filter(s.chk,nil,TYPE_FUSION)
	local g2=rg:Filter(s.chk,nil,TYPE_PENDULUM)
	local g3=rg:Filter(s.chk,nil,TYPE_SYNCHRO)
	local g4=rg:Filter(s.chk,nil,TYPE_XYZ)
	local g=g1:Clone()
	g:Merge(g2)
	g:Merge(g3)
	g:Merge(g4)
...
function s.zarcop(e,tp,eg,ep,ev,re,r,rp)
	local rg=Duel.GetMatchingGroup(Card.IsAbleToRemove,tp,ZARC_LOC,0,nil):Filter(s.filter,nil,TYPE_FUSION,TYPE_PENDULUM,TYPE_SYNCHRO,TYPE_XYZ)
	...
	if Duel.Remove(g,POS_FACEUP,REASON_EFFECT)>3 then
		...
		if tc and Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)>0 and tc:IsOriginalCode(4206920) then
