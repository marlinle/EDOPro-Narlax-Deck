...
function s.rescon(sg,e,tp,mg)
	return sg:IsExists(s.chk,1,nil,sg,Group.CreateGroup(),41209827,82044279,16195942,16178681) and Duel.GetLocationCountFromEx(tp,tp,sg,TYPE_FUSION)>0
end
function s.zarctg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g1=rg:IsAttribute(ATTRIBUTE_LIGHT,fc,sumtype,tp) and c:IsType(TYPE_FUSION,fc,sumtype,tp)
	local g2=rg:IsAttribute(ATTRIBUTE_LIGHT,fc,sumtype,tp) and c:IsType(TYPE_SYNCHRO,fc,sumtype,tp)
	local g3=rg:IsAttribute(ATTRIBUTE_LIGHT,fc,sumtype,tp) and c:IsType(TYPE_XYZ,fc,sumtype,tp)
	local g4=rg:IsAttribute(ATTRIBUTE_LIGHT,fc,sumtype,tp) and c:IsType(TYPE_PENDULUM,fc,sumtype,tp)
	local g=g1:Clone()
	g:Merge(g2)
	g:Merge(g3)
	g:Merge(g4)
