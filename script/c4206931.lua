...
s.listed_series={0x420b}
function s.cfilter(c,tp,typ)
	return c:IsFaceup() and c:IsType(typ) and c:IsSetCard(0x420b) and c:IsControler(tp) and c:IsSummonLocation(LOCATION_EXTRA)
end
...
function s.filter(c,tp,typ)
	return c:IsType(typ) and c:IsSetCard(0x420b) and c:IsControler(tp) and c:IsSummonLocation(LOCATION_EXTRA)
end
...
function s.rmfilter(c)
	return c:IsAbleToRemove() and (c:IsLocation(LOCATION_PZONE) or aux.SpElimFilter(c,false,true))
end
